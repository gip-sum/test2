...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ mode?: string; next?: string; error?: string }> }) {
  ...
  return <PageShell><div className="mx-auto max-w-lg px-4 py-10 sm:py-16"><LoginForm mode={mode === 'register' ? 'register' : 'login'} next={returnPath} configured={Boolean(authConfig())} googleConfigured={googleAvailable()} googleError={error === 'google'} /></div></PageShell>
}
